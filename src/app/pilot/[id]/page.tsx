...
import { simulateWinChance } from "@/lib/battle-engine";
...
export default async function PilotProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ── Snoop Analysis: simulate win chance ──
  const isOwnProfile = pilot.userId === session.user.id;
  let snoopAnalysis: { winPct: number; avgRounds: number; avgLfRemaining: number } | null = null;

  if (!isOwnProfile) {
    const viewer = await getOrCreatePilotState(session.user.id, session.user.name);
    const viewerInventory = await prisma.inventoryItem.findMany({
      where: { pilotId: viewer.id },
      select: { type: true, bonusType: true, bonusAmt: true, equipped: true },
    });

    snoopAnalysis = simulateWinChance(
      {
        callsign: viewer.callsign,
        level: viewer.level,
        lifeForce: viewer.lifeForce,
        strength: viewer.strength,
        speed: viewer.speed,
        confidence: viewer.confidence,
        atkSplit: viewer.atkSplit,
        characterSlug: viewer.characterSlug,
      ...
      },
      {
        ...
        inventory: pilot.inventory.map((i) => ({ type: i.type, bonusType: i.bonusType, bonusAmt: i.bonusAmt, equipped: i.equipped })),
      }
    );
  }
