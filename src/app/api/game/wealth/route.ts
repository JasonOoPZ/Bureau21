...
const WEALTH_RATES: Record<number, { rate: number; name: string }> = {
  7:   { rate: 5,   name: "Treasury Notes" },
  14:  { rate: 10,  name: "Municipal Bonds" },
  30:  { rate: 18,  name: "Blue-Chip Real Estate" },
  45:  { rate: 28,  name: "Private Credit" },
  60:  { rate: 38,  name: "Hedge Fund LP" },
  90:  { rate: 50,  name: "Private Equity" },
  180: { rate: 65,  name: "Venture Capital" },
  365: { rate: 90,  name: "Sovereign Wealth Fund" },
};
...
export async function POST(request: Request) {
  ...
  if (action === "buy_investment") {
    ...
    const [updated, newInvestment] = await prisma.$transaction([
  ...
  if (action === "claim_yield") {
    ...
    const totalDaysPassed = Math.min(
      Math.floor((now.getTime() - inv.createdAt.getTime()) / 86400000),
      inv.days,
    );
    const daysAlreadyClaimed = Math.floor(
      (inv.lastClaimedAt.getTime() - inv.createdAt.getTime()) / 86400000,
    );
  ...
  if (action === "collect_investment") {
    ...
    // Calculate unclaimed remaining yield
    const daysAlreadyClaimed = Math.floor(
      (inv.lastClaimedAt.getTime() - inv.createdAt.getTime()) / 86400000,
    );
